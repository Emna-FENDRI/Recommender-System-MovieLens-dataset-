...
package distributed {
  ...
  object Approximate {
    def main(args: Array[String]) = {
      ...
      val conf_replication = conf.replication()
      val conf_partitions = conf.partitions()
      ...
      val neighboursMatrix =
        buildApproximateSimMatrix(train, conf_partitions, conf_replication, sc, conf_users, conf_movies,conf_k,globalAverage,usersAvg,normalizedDev)

      val approximatePredictor =
        predictorKnnApproximate(train, conf_k , sc, conf_users, conf_movies, conf_partitions, conf_replication)



      val measurements = (1 to conf.num_measurements()).map(_ =>
        timingInMs(() => {
          getMAE(predictorKnnApproximate(train, conf_k, sc, conf_users, conf_movies, conf_partitions, conf_replication), test)
        })
      )
      ...
      conf.json.toOption match {
        ...
        case Some(jsonFile) => {
          val answers = ujson.Obj(
            ...
            "AK.1" -> ujson.Obj(
              "knn_u1v1" -> ujson.Num(neighboursMatrix(0, 0)),
              "knn_u1v864" -> ujson.Num(neighboursMatrix(0, 863)),
              "knn_u1v344" -> ujson.Num(neighboursMatrix(0, 343)),
              "knn_u1v16" -> ujson.Num(neighboursMatrix(0, 15)),
              "knn_u1v334" -> ujson.Num(neighboursMatrix(0, 333)),
              "knn_u1v2" -> ujson.Num(neighboursMatrix(0, 1))
            ),
            "AK.2" -> ujson.Obj(
              "mae" -> ujson.Num(mae)
            ),
