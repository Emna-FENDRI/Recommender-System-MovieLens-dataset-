...
class BaselineTests extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("Compute global average") {
    assert(within(cmptGlobAvg(train2), 3.5264625, 0.0001))
  }
  ...
  test("Compute item 1 average deviation") {
    ...
    assert(
      within(
        computeAvgDevs(train2, computeUserAvg(train2), cmptGlobAvg(train2))
          .getOrElse(1, 0.0),
        0.3027,
        0.0001
      )
  ...
  test("Compute baseline prediction for user 1 on item 1") {
    assert(within(predictorBaseline(train2)(1, 1), 4.0468, 0.0001))
  }
  ...
  // with the following signature:ss;
  ...
  test(
    ...
    assert(
      within(
        getMAE(predictorGlobAvg(train2), test2),
        0.9489,
        0.0001
      )
    )
    assert(
      within(
        getMAE(predictorUserAvg(train2), test2),
        0.8383,
        0.0001
      )
    )
    assert(
      within(
        getMAE(predictorItemAvg(train2), test2),
        0.8206,
        0.0001
      )
    )
    assert(
      within(
        getMAE(predictorBaseline(train2), test2),
        0.7604,
        0.0001
      )
    )
