...
object DistributedBaseline extends App {
  ...
  val measurements = (1 to conf.num_measurements()).map(x =>
    timingInMs(() => {
      getMaeRDD(predictorBaselineRDD(train), test)
    })
  )
  ...
  conf.json.toOption match {
    ...
    case Some(jsonFile) => {
      val answers = ujson.Obj(
        ...
        "D.1" -> ujson.Obj(
          "1.GlobalAvg" -> ujson.Num(
            cmptGlobAvgRDD(train)
          ), // Datatype of answer: Double
          "2.User1Avg" -> ujson.Num(
            userAvgRDD(train, 1)
          ), // Datatype of answer: Double
          "3.Item1Avg" -> ujson.Num(
            itemAvgRDD(train, 1)
          ), // Datatype of answer: Double
          "4.Item1AvgDev" -> ujson.Num(
            computeAvgDevsRDD(
              train,
              computeUserAvgsRDD(train),
              cmptGlobAvgRDD(train)
            ).getOrElse(1, 0.0)
          ), // Datatype of answer: Double,
          "5.PredUser1Item1" -> ujson.Num(
            predictorBaselineRDD(train)(1, 1)
          ), // Datatype of answer: Double
          "6.Mae" -> ujson.Num(
            getMaeRDD(predictorBaselineRDD(train), test)
          ) // Datatype of answer: Double
